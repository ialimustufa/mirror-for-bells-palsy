...
function referenceStandardControlLines(validation = {}, readiness = {}) {
  ...
  return [
    ...
    `- Estimate evidence control: counted rows require Mirror estimates with status \`estimated\`, complete/minimum evidence tier, at least ${Math.round(minUsableMovementCoverageRatio * 100)}% usable movement coverage, used/omitted movement IDs, and the usable-movements-only calculation flag. Scale-specific rows with missing or invalid estimates are reported in that scale's denominator as missing estimates.`,
...
function buildClinicalScaleAgreementMarkdown(input = {}, options = {}) {
  ...
  const lines = [
    ...
    "- Estimator input provenance: counted v3 rows preserve used/omitted movement IDs and the usable-movements-only calculation flag.",
    "",
    "## Dataset Summary",
    "",
