...
function referenceStandardControlLines(validation = {}, readiness = {}) {
  ...
  return [
    ...
    `- Estimate evidence control: counted rows require Mirror estimates with status \`estimated\`, complete/minimum evidence tier, at least ${Math.round(minUsableMovementCoverageRatio * 100)}% usable movement coverage, used/omitted movement IDs, the usable-movements-only calculation flag, complete resting-metric keys, and the complete-resting-metrics calculation flag. House-Brackmann estimates require the gentle eye-closure input. Scale-specific rows with missing or invalid estimates are reported in that scale's denominator as missing estimates.`,
