...
test("validation label sheet exports frame sample and clinical scale label rows", () => {
  ...
  assert.equal(rows[1].reviewerId, "");
  ...
  assert.equal(rows[1].estimateCalculationUsesCompleteRestingMetrics, "true");
...
test("validation label sheet can hide Mirror estimates for blinded review", () => {
  ...
  assert.equal(clinicalRow[index.reviewerId], "");
  ...
  assert.equal(clinicalRow[index.estimateCalculationUsesCompleteRestingMetrics], "true");
...
test("validation label merge updates reviewed fields from CSV", () => {
  const csv = [
    "rowType,sampleId,assessmentId,validationCaseId,quality,visibleMovementLevel,coactivationNotes,houseBrackmannGrade,sunnybrookComposite,efaceTotal,efaceStatic,efaceDynamic,efaceSynkinesis,clinicianConfidence,sourceLabelSheetMode,reviewBlinded,labelSource,reviewerId,reviewerRole,reviewedAt,notes",
    'frameSample,sample-1,,,strong,moderate,"eye relaxed, mouth moved",,,,,,,,,,,reviewer-frame-1,clinician,2026-06-23T10:00:00.000Z,"usable label"',
    "assessmentClinicalScale,,assessment-1:clinical-scale,case-001,,,,II,82,79,91,77,69,high,blinded,yes,clinician-assigned,reviewer-clinical-1,clinician,2026-06-23T10:01:00.000Z,reviewed scale labels",
  ].join("\n");
  ...
  assert.equal(label.reviewerId, "reviewer-frame-1");
  ...
  assert.equal(clinicalLabel.reviewerId, "reviewer-clinical-1");
