...
- Phase 0 instrumentation: implemented on `algorithm-upgrade`. Sessions, exercise records, movement profiles, and frame-sample scoring payloads now carry `scoringModelVersion`; live scoring stores structured `dropReason` counts and per-rep score distributions.
...
- Phases 2-5: pending. These require standardized assessment records, clinical-style reports, reliability-aware personalization, validation datasets, and release gates.
...
- Do not claim a House-Brackmann, Sunnybrook, or eFACE grade unless the scoring is clinically validated.
