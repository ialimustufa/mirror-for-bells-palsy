...
- Phase 5 release gates: implemented with `npm run release:check`, rollback-safe backup parse tests, a machine-readable `docs/validation-status.json`, runtime clinical-scale presentation checks that fail closed if the documented 80% standard, review-package verification, or case-mix floors are weakened, and documentation checks for medical, privacy, and validation status.
