...
generation dates. Clinical-scale agreement and threshold calibration artifacts
must also include `sourceDatasetSha256`, and `npm run validation:status` rejects
enabled clinical-facing support unless each enabled per-scale status entry
repeats the agreement hash, names the listed passed clinical review package
verification report for the blinded source package, and lists threshold
